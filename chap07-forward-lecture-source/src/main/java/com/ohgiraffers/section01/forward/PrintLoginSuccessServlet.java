...
public class PrintLoginSuccessServlet extends HttpServlet {
    ...
    /*
    * forward할 떄 전달한 request 와 response의 모든 정보를 이용해 새로운 request와 response를
    * 만들고, 그 정보를 이용해 다시 http 메소드에 맞는 서블릿의 doGet 혹은 doPost를 요청하는 방식
    * 깊은 복사 이용해 값을 그대로 복사했기 때문에 내부에 존재하는 헤더나 인스턴스는 그대로 유지하고
    * */
    @Override
    ...
    * */
        ...
        * 저장하여, forward하면 위임받은 서블릿에서도 위임한 서블릿의 정보를 공유 할 수 있다.
        * 이때 forward 받은 서블릿의 존재를 클라이언트가 알 필요가 없기떄문에
        * URL이 변경되지 않는다. (결과화면만 받으면 되기 떄문)
