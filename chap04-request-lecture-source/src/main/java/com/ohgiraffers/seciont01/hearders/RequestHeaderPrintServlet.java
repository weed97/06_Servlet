package com.ohgiraffers.seciont01.hearders;
...
public class RequestHeaderPrintServlet extends HttpServlet {
    ...
    * 해더의 종류 4가지
    * 1. General header
    *       요청 및 응답 모두에 적용되지만 최종적으로 body의 전송되는 것과 관련이 없는 헤더
    *       body : 클라이언트가 요청을 보낼때 POST에 이전 특정 값을 담는 공간
    * 2. Request header
    *       Fetch될 리소스나 클라이언트 자체에 대한 상세 정보를 포함하는 헤더
    * 3. Response header
    *       위치나 서버 자체의 같은 응답에 대한 부가적인 정보를 갖는 헤더
    * 4. Entity header
    *       컨텐츠의 길이나 MIME타입과 같은 엔티티 body의 대한 상세 정보를 포함하는 헤더
    *       (요청 응답 모두 사용되며, 메세지 바디의 컨텐츠를 나타내기에 GET요청은 해당되지 않는다.)
    * */
    @Override
