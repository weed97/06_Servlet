...
@WebServlet(value = "/annotation-lifecycle") // http:localhost:8080/annotation-lifeservlet
public class LifeCycleTestServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        * 두 번쨰 요청부터는 init() 호출 없이 바로 service()를 호출한다.
        * */
        System.out.println("annotation 매핑 service() 메소드 호출 " + serviceCount++);
    }

    // 서블릿 컨테이너가 종료될때 종료되는 메소드, 주로 자원이 반납하는 용도로 사용된다.
    @Override
    public void destroy() {
